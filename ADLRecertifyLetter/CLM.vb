...
Namespace Se2.ISCORSP.Client.GLCO.ADLRecertifyLetter.CLM
...
#Region " POPULATION FUNCTIONS "
        ...
        Public Function ANNUAL_LIMIT_PYMT(ByVal value As Se2Args) As String
            ...
            If Se2Source.SP_MODE_CODE = "M" Then
                retval = Math.Round((Se2Source.GWB_ANNUAL_LIMIT / 12), 2).ToString
            ElseIf Se2Source.SP_MODE_CODE = "S" Then
                retval = Math.Round((Se2Source.GWB_ANNUAL_LIMIT / 2), 2).ToString
            ElseIf Se2Source.SP_MODE_CODE = "Q" Then
                retval = Math.Round((Se2Source.GWB_ANNUAL_LIMIT / 4), 2).ToString
            ElseIf Se2Source.SP_MODE_CODE = "A" Then
                retval = Math.Round((Se2Source.GWB_ANNUAL_LIMIT / 1), 2).ToString
            ElseIf Se2Source.SP_MODE_CODE = "B" Then
                retval = Math.Round((Se2Source.GWB_ANNUAL_LIMIT / 6), 2).ToString
            Else retval = ""
            End If
        ...
        End Function
        Public Function GetPayMode(ByVal value As Se2Args) As String
            Select Case True
                Case Se2Source.SP_MODE_CODE = "M"
                    Return "monthly"
                Case Se2Source.SP_MODE_CODE = "Q"
                    Return "quarterly"
                Case Se2Source.SP_MODE_CODE = "A"
                    Return "annually"
                Case Se2Source.SP_MODE_CODE = "S"
                    Return "semiannually"
                Case Se2Source.SP_MODE_CODE = "SM"
                    Return "bi-monthly"
                Case Else
                    Return ""
            End Select
        ...
        Public Function GetCertifyDate(ByVal value As Se2Args) As String


            Dim today As Date = Date.Now
            Dim regDate As Date = Convert.ToDateTime(Se2Source.ISSUE_DATE)

            ''Dim correctDate = DateSerial(today.Year, regDate.Month, regDate.Day)
            'Dim correctD As Date = Date.Parse(correctDate)
            Dim FinalDate As Date = (DateAdd(DateInterval.Day, -25, regDate))
            FinalDate = DateSerial(today.Year, FinalDate.Month, FinalDate.Day)
            Return CType(FinalDate.ToString("yyyy-MM-dd"), String)
...
#Region " POPULATION FUNCTIONS "
        ...
        Public Function GetEmailAddress(ByVal value As Se2Args) As Object
            ...
            If CBool(Se2Source.MAIL_FLAG) Then
                ...
                StoreEmail.addParms("@DocumentType", "tslt")
